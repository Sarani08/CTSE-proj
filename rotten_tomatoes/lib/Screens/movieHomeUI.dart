...
class _MovieHomeUIState extends State<MovieHomeUI> {
  ...
  Future getData() async {
    http.Response response = await http.get(
        "https://api.nytimes.com/svc/movies/v2/reviews/all.json?&api-key=kq71eQPzi5qiOMAHNNc8aTOiBrYkvYWL");
          ...
          return new GestureDetector(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ListTile(
                    ...
                    trailing: Text(
                      "${userData[index]["critics_pick"]}",
                      style: TextStyle(color: Colors.red),
                    ),
                    leading:
                        Image.network(userData[index]["multimedia"]["src"]),
                  ),
                ],
              ),
            ),
            onTap: () {
              ...
              Navigator.push(context, new MaterialPageRoute(builder: (context) {
                return new MovieDetailUI(
                    imageURl, title, summary, date, author, critics, id);
              }));
            },
          );
        },
